    ...
    try:
        ...
        for line in STATIC_1302_HERO_ROWS.splitlines():
            ...
            chars[str(config_id)]={'name':name,'gdid':int(gdid),'code':code,'soul_sign':int(soul),
                ...
                # CharacterConfig extraction: every hero starts with PRE 10 / RES 0.
                # These two fields were reversed in the 0.60.1302 supplement.
    ...
    for config_id,row in chars.items():
        ...
        HERO_PROGRESSION[str(config_id)]={'name':row.get('name'),'soul_sign':int(sign),'nodes':nodes,
          ...
          'awake_base':{'health':ab.get('health'),'atk':ab.get('attack'),'defense':ab.get('defense'),'run_speed':ab.get('run_speed'),
                        ...
                        'instinct':ab.get('mana_generation'),'combo_points':ab.get('instinct'),
                        'skill_speed_points':ab.get('combo_speed'),'skill_recovery_points':ab.get('skill_speed'),
                        'mana_points':ab.get('skill_recovery')}}
...
</style></head><body><header><h1>🧪 Smishie's Lab</h1><div class=muted>V10.98 — import Salle séparé</div></header>
...
if __name__=='__main__':
    print("Smishie's Lab V10.98 — import Salle séparé — http://127.0.0.1:8501")
