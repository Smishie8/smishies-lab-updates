    ...
    def do_GET(self):
        ...
        try:
            ...
            if p.path=='/api/aoe-rank':
                preset=qs.get('preset',['box'])[0]; duration=f('duration',60); enemies=max(1,int(f('enemies',20))); defense=f('defense',0); resistance=f('resistance',0); element=qs.get('element',['Neutre'])[0]
                out=[]
