...
try:
    from boss_engine import get_boss_profile as _get_boss_profile, profile_as_dict as _boss_profile_as_dict
except Exception:
...
<h3>Résumé combat</h3><div id=combatSummary class=grid></div><h3>Apport des buffers</h3><div id=supportContribution class=scroll></div><h3>Buffs équipe / débuffs boss</h3><div id=teamBuffTable class=scroll></div><div id=combatNote class=note></div><h3>Timeline globale (prototype)</h3><div class=note>Le carry et le boss partagent ici la même horloge. Les actions boss sont déjà placées avec leurs timings exacts ; leurs dégâts/débuffs seront branchés sur les états de l'équipe à l'étape suivante.</div><div id=globalCombatTable class=scroll></div><h3>Timeline du carry</h3><div id=combatTable class=scroll></div></div>
            ...
            if p.path=='/api/combat':
                ...
                if r:
                    ...
                    if _get_boss_profile and _boss_profile_as_dict:
                        ...
                        if _bp:r['boss_dynamic']=_boss_profile_as_dict(_bp,{})
