    ...
    for config_id,row in chars.items():
        ...
        HERO_PROGRESSION[str(config_id)]={'name':row.get('name'),'soul_sign':int(sign),'nodes':nodes,
          ...
          'awake_base':{'health':ab.get('health'),'atk':ab.get('attack'),'defense':ab.get('defense'),'run_speed':ab.get('run_speed'),
                        'crit_rate':ab.get('critical_rate'),'crit_dmg':ab.get('critical_damage'),'accuracy':ab.get('resistance'),
                        'resistance':ab.get('accuracy'),'instinct':ab.get('instinct'),'combo_points':ab.get('combo_speed'),
...
def ensure_box_tables():
    with sqlite3.connect(DB) as con:
        ...
        # V10.85: niveaux personnels de la Salle des trophées (Great Hall), importés depuis PlayerArenaModel.dat.
...
def box_build_for_instance(name,hero_instance):
    ...
    # V10.85: les valeurs de fiche doivent correspondre aux nombres affichés par le jeu.
    # Les tests utilisateur montrent que ces nombres correspondent à la reconstruction
    # base + éveil + reliques; la Salle des trophées est conservée séparément.
    b=relic_b
    ...
    if cfg:
        ...
        if 1 <= level <= len(curve):
            ...
            for node_id in node_ids:
                ...
                # V10.85: awake_nodes.json currently mislabels some node bonuses as ManaGeneration.
...
def profile_stats(name,p=None,include_hall=False):
    ...
    return _apply_hall_effective_stats(name,st) if include_hall else st
...
</style></head><body><header><h1>🧪 Smishie's Lab</h1><div class=muted>V10.85 — Salle des trophées séparée des stats de fiche</div></header>
...
if __name__=='__main__':
    print("Smishie's Lab V10.85 — Salle des trophées séparée des stats de fiche — http://127.0.0.1:8501")
