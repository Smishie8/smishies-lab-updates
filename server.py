...
def box_build_for_instance(name,hero_instance):
    ...
    if cfg:
        ...
        if 1 <= level <= len(curve):
            ...
            naked={'health':round((num(base.get('health'))+num(awb.get('health')))*10*mult),
                   ...
                   'mana_gen':(num(base.get('mana_points'))+num(awb.get('mana_points')))/1000.0}
            ...
            for node_id in node_ids:
                ...
                naked['mana_gen']+=num(n.get('mana_points'))/1000.0
    ...
    final={
        ...
        'mana_gen':num(h.get('mana_gen'))+b['mana_points']/1000.0,
    }
...
def _stats_without_relics(name, raw_profile, current_relics, profile_is_saved):
    ...
    return {
        ...
        'mana_gen':max(0.0,num(raw_profile.get('mana_gen'))-b['mana_points']/1000.0),
    }
...
</style></head><body><header><h1>🧪 Smishie's Lab</h1><div class=muted>V10.78 — Stats secondaires box corrigées</div></header>
...
if __name__=='__main__':
    print("Smishie's Lab V10.78 — Stats secondaires box corrigées — http://127.0.0.1:8501")
