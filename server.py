...
try:
    from boss_engine import (
        get_boss_profile as _get_boss_profile,
        profile_as_dict as _boss_profile_as_dict,
        simulate_opening_survival as _simulate_opening_survival,
    )
except Exception:
    _get_boss_profile=None
    _boss_profile_as_dict=None
    _simulate_opening_survival=None
...
try:
    from support_eval.support_evaluator_v2 import (
        load_support_profiles_from_db as _load_support_eval_profiles,
        coverage_summary as _support_coverage_summary,
    ...
    _support_coverage_summary=None
...
SUPPORT_EVAL_OVERRIDES=os.path.join(BASE,'support_eval','support_effect_overrides.json')
...
def _load_arena_hall_prices():
    """Load Great Hall upgrade prices and medal conversions extracted from StaticArenaData."""
    try:
        with open(ARENA_STATIC_FILE,'r',encoding='utf-8') as f:
            d=json.load(f) or {}
        out={}
        for row in (d.get('hall_prices') or []):
            lv=int(row.get('target_level') or 0)
            if not (1<=lv<=15): continue
            out[lv]={
                'resource_id':int(row.get('resource_id') or 0),
                'amount':float(row.get('amount') or 0),
                'gold_equivalent':float(row.get('gold_equivalent') or 0),
...
ARENA_HALL_PRICES=_load_arena_hall_prices()
...
def _trophy_sim_audit(name, sim, levels):
    ...
    out={'casts':casts,'damage_by':dict((sim or {}).get('damage_by') or {}),'dot_by_source':dict((sim or {}).get('dot_by_source') or {}),'dot_tick_details':dict((sim or {}).get('dot_tick_details') or {})}
...
def optimize_trophy_hall(element, hero_names=None, duration=120, boss_def=1320, boss_res=0, boss_hp=0, boss_atk=0, boss_element='Neutre', costs=None):
    ...
    for x in (costs or []):
        try:
            v=max(0.0,float(x)); parsed_costs.append(v)
            if v>0: manual_costs=True
        except: parsed_costs.append(0.0)
    while len(parsed_costs)<15: parsed_costs.append(0.0)
    if not manual_costs:
        parsed_costs=[float((ARENA_HALL_PRICES.get(lv) or {}).get('gold_equivalent') or 0.0) for lv in range(1,16)]
    ...
    for name in names:
        ...
        p=profile_for(name)
        # Trophy Hall optimizer must start from the exact same final stats shown
        # on the Hero sheet. This avoids divergence between a saved/imported profile
        # and a freshly reconstructed box_build.
    ...
    for sid,(label,key) in TROPHY_DPS_STATS.items():
        ...
        if cur>=15: continue
        nxt=cur+1
        price_info=ARENA_HALL_PRICES.get(nxt) or {}
        cost=parsed_costs[nxt-1] if nxt-1<len(parsed_costs) else 0.0
        raw_cost=float(price_info.get('amount') or 0.0) if not manual_costs else 0.0
        raw_resource_id=int(price_info.get('resource_id') or 0) if not manual_costs else 0
        raw_resource_label=str(price_info.get('resource_label') or '') if not manual_costs else ''
        ...
        for name in usable:
            ...
            hero_rows.append({'hero':name,'before_dps':round(b['dps'],2),'after_dps':round(d2,2),'gain_dps':round(gain,2),'gain_pct':pct,
                              'before_stats':{k:num(b['stats'].get(k)) for k in ('atk','crit_rate','crit_dmg','accuracy','combo_speed','skill_speed','skill_recovery','mana_gen')},
                              'after_stats':{k:num(st2.get(k)) for k in ('atk','crit_rate','crit_dmg','accuracy','combo_speed','skill_speed','skill_recovery','mana_gen')},
        ...
        rows.append({'stat_id':sid,'stat':label,'current_level':cur,'next_level':nxt,
                     ...
                     'cost_gold_equivalent':cost,'cost_raw':raw_cost,'cost_resource_id':raw_resource_id,
                     'cost_resource_label':raw_resource_label,'cost_source':'manual' if manual_costs else 'StaticArenaData.HallPrices',
    ...
    return {'ok':True,'element':element,'heroes':usable,'hero_count':len(usable),'duration':duration,
            'base_total_dps':round(base_total,2),'metric':metric,'costs':parsed_costs,
            'cost_source':'manual' if manual_costs else 'StaticArenaData.HallPrices',
            'hall_prices':ARENA_HALL_PRICES,'rows':rows}
...
def survival_stats_for(name, override=None):
    """Return HP/DEF/RES for incoming-damage simulation.

    Prefer explicit override values, then exact imported box stats.  We do not
    invent HP/DEF for unresolved characters.
    """
    ov=override or {}
    if num(ov.get('health'))>0 and ov.get('defense') is not None:
        return {
            'name':name,
            'health':num(ov.get('health')),
            'defense':num(ov.get('defense')),
            'resistance':num(ov.get('resistance')),
            'source':'preset/custom'
        }
    bx=box_hero_for_name(name) or {}
    fs=((bx.get('box_build') or {}).get('final_stats') or {})
    if num(fs.get('health'))>0 and fs.get('defense') is not None:
        return {
            'name':name,
            'health':num(fs.get('health')),
            'defense':num(fs.get('defense')),
            'resistance':num(fs.get('resistance')),
            'source':'box_exact'
        }
...
def support_buff_schedule(name, duration=120, adds_mode='none', boss_element='Neutre', support_mode='real', stats_override=None, levels_override=None):
    ...
    ready={'s1':0.0,'s2':0.0,'s3':0.0,'ult':0.0}; mana=0.0; auto_idx=1; t=0.0; seq=0; occ={}; events=[]; casts=[]
    while t<duration and seq<3000:
        ...
        start=t; occ[action]=occ.get(action,0)+1; casts.append({'time':start,'key':key,'action':action})
...
def prepare_team_buffs(supports,duration,adds_mode='none',boss_res=0,boss_element='Neutre',support_mode='real'):
    ...
    for n in supports or []:
        ...
        z=support_buff_schedule(n,duration,adds_mode,boss_element,support_mode); raw.extend(z['events']); support_info.append({'name':n,'actions':z['actions'],'casts':z.get('casts',[]),'buff_casts':sum(1 for e in z['events'] if e.get('kind')=='buff'),'debuff_attempts':sum(1 for e in z['events'] if e.get('kind')=='debuff_attempt'),'stats':z.get('stats',{}),'levels':z.get('levels',{}),'element':z.get('element','Neutre'),'element_matchup':z.get('element_matchup',{})})
...
def support_eval_profiles():
    """Charge une fois par analyse les profils Support pour enrichir /api/best-supports.
    Lecture seule de invokers.db. Retourne {} si le module optionnel est absent.
    """
    if _load_support_eval_profiles is None:
        return {}
    try:
        return _load_support_eval_profiles(DB,SUPPORT_EVAL_OVERRIDES)
    except Exception:
        return {}

def support_eval_payload(profile,boss_name=''):
    if profile is None:
        return None
    coverage=_support_coverage_summary(profile) if _support_coverage_summary else {}
    advanced=[]
    ...
    payload={
        ...
        'score_boss_100':None,
        'survival_model_ready':bool(coverage.get('ready_for_precise_survival_scoring')),
    }
    if boss_name and 'ulgorim' in str(boss_name).lower() and _support_boss_mechanic_notes:
        try: payload['boss_notes']=_support_boss_mechanic_notes(profile)
        except Exception: payload['boss_notes']=[]
    else:
        payload['boss_notes']=[]
    return payload
...
<h3>Équipe dynamique — 5 héros</h3><div class=note>La tactique complète du jeu n'est pas encore décodée. Pour l'instant, choisis la ligne de chaque héros. Les bonus de classe/slot restent désactivés tant qu'on ne les a pas extraits.</div><div id=dynamicTeam class=levels></div><div class=controls><div class=control><label>Poids Front</label><input id=aggroFront type=number step=.1 value=5></div><div class=control><label>Poids Mid</label><input id=aggroMid type=number step=.1 value=2></div><div class=control><label>Poids Back</label><input id=aggroBack type=number step=.1 value=1></div></div>
...
<div class=controls><div class=control><label>Nombre de supports affichés</label><select id=bestSupportLimit><option value=3>Top 3</option><option value=5>Top 5</option><option value=10>Top 10</option><option value=20>Top 20</option></select></div></div><h3 id=bestSupportTopTitle>Top 3 individuels</h3><div id=bestSupportTop class=scroll></div>
...
<h3>Coûts des niveaux</h3>
<div class=note><b>Coûts automatiques :</b> Smishie's Lab utilise maintenant directement <b>StaticArenaData.HallPrices</b>. Le rendement est normalisé en équivalent médaille d'or pour comparer correctement Bronze, Argent et Or. Aucun coût manuel n'est nécessaire.</div>
<div class=controls><button id=trophyBtn>Optimiser la Salle</button></div>
...
async function trophyRun(){
  ...
  try{
    ...
    trophySummary.innerHTML=cards({'Élément':d.element,'Héros simulés':d.hero_count,'DPS total actuel':F(d.base_total_dps),'Classement':d.metric==='dps_per_medal'?'DPS / médaille or équiv.':'Gain DPS brut','Coûts':d.cost_source||'—'});
    trophyTable.innerHTML='<table><tr><th>#</th><th>Stat</th><th>Niveau</th><th>Gain stat</th><th>Coût jeu</th><th>Équiv. or</th><th>Gain DPS total</th><th>Gain moyen</th><th>DPS / médaille or</th><th>Principal bénéficiaire</th><th>Audit rotation</th></tr>'
      +d.rows.map((r,i)=>{let top=(r.heroes||[])[0];let audit='—';if(top){let a=top.before_audit||{},z=top.after_audit||{},ac=a.casts||{},zc=z.casts||{};audit=`S1 ${ac['Skill 1']||0}→${zc['Skill 1']||0} · S2 ${ac['Skill 2']||0}→${zc['Skill 2']||0} · S3 ${ac['Skill 3']||0}→${zc['Skill 3']||0} · Ult ${ac['Ultimate']||0}→${zc['Ultimate']||0}`;if(top.hero==='Brandis')audit+=` · Ult×3 ${a.brandis_ult_x3||0}→${z.brandis_ult_x3||0}`;}return `<tr data-trophy-row="${i}"><td>${i+1}</td><td><b>${r.stat}</b></td><td>${r.current_level}→${r.next_level}</td><td>${r.delta_value==null?'—':([4,7,8].includes(+r.stat_id)?('+'+F1(r.delta_value*100)+' %'):('+'+F1(r.delta_value)))}</td><td>${r.cost_raw>0?(F1(r.cost_raw)+' '+(r.cost_resource_label||('ID '+r.cost_resource_id))):'—'}</td><td>${r.cost_gold_equivalent>0?F1(r.cost_gold_equivalent):'—'}</td><td class=good>+${F1(r.gain_total_dps)}</td><td>${P(r.gain_avg_pct)}</td><td class=good>${r.dps_per_medal==null?'—':F1(r.dps_per_medal)}</td><td>${top?top.hero+' (+'+F1(top.gain_dps)+')':'—'}</td><td>${audit}</td></tr>`}).join('')+'</table>';
    let renderDetail=(idx)=>{let r=d.rows[idx];if(!r)return;trophyHeroDetail.innerHTML='<h4>'+r.stat+' '+r.current_level+'→'+r.next_level+'</h4><table><tr><th>Héros</th><th>DPS avant</th><th>DPS après</th><th>Gain DPS</th><th>Gain %</th><th>PRÉ</th><th>Recovery</th><th>S1</th><th>S2</th><th>S3</th><th>Ult</th><th>Brandis Ult ×3</th><th>DoT S2</th><th>Ticks S2</th><th>DoT S3</th><th>Ticks S3</th><th>DoT Ult</th><th>Ticks Ult</th></tr>'+(r.heroes||[]).map(x=>{let a=x.before_audit||{},z=x.after_audit||{},ac=a.casts||{},zc=z.casts||{};let tri=(x.hero==='Brandis')?((a.brandis_ult_x3||0)+'→'+(z.brandis_ult_x3||0)):'—';return `<tr><td>${x.hero}</td><td>${F1(x.before_dps)}</td><td>${F1(x.after_dps)}</td><td class=good>+${F1(x.gain_dps)}</td><td>${P(x.gain_pct)}</td><td>${F1((x.before_stats||{}).accuracy)}→${F1((x.after_stats||{}).accuracy)}</td><td>${P((x.before_stats||{}).skill_recovery)}→${P((x.after_stats||{}).skill_recovery)}</td><td>${ac['Skill 1']||0}→${zc['Skill 1']||0}</td><td>${ac['Skill 2']||0}→${zc['Skill 2']||0}</td><td>${ac['Skill 3']||0}→${zc['Skill 3']||0}</td><td>${ac['Ultimate']||0}→${zc['Ultimate']||0}</td><td>${tri}</td><td>${x.hero==='Brandis'?F1(((a.dot_tick_details||{})['Skill 2']||{}).damage||0)+'→'+F1(((z.dot_tick_details||{})['Skill 2']||{}).damage||0):'—'}</td><td>${x.hero==='Brandis'?((((a.dot_tick_details||{})['Skill 2']||{}).ticks||0)+'→'+(((z.dot_tick_details||{})['Skill 2']||{}).ticks||0)):'—'}</td><td>${x.hero==='Brandis'?F1(((a.dot_tick_details||{})['Skill 3']||{}).damage||0)+'→'+F1(((z.dot_tick_details||{})['Skill 3']||{}).damage||0):'—'}</td><td>${x.hero==='Brandis'?((((a.dot_tick_details||{})['Skill 3']||{}).ticks||0)+'→'+(((z.dot_tick_details||{})['Skill 3']||{}).ticks||0)):'—'}</td><td>${x.hero==='Brandis'?F1(((a.dot_tick_details||{})['Ultimate']||{}).damage||0)+'→'+F1(((z.dot_tick_details||{})['Ultimate']||{}).damage||0):'—'}</td><td>${x.hero==='Brandis'?((((a.dot_tick_details||{})['Ultimate']||{}).ticks||0)+'→'+(((z.dot_tick_details||{})['Ultimate']||{}).ticks||0)):'—'}</td></tr>`}).join('')+'</table>'};
    renderDetail(0);
...
let combatCustomLoadedHero='';
function renderDynamicTeam(){let defaults=[combatHero.value,support1.value,support2.value,support3.value,support4.value].map(x=>x==='Aucun'?'':x);let lanes=['front','front','mid','back','back'];dynamicTeam.innerHTML=[0,1,2,3,4].map(i=>`<div class=control><label>Slot ${i+1}</label><select id=teamHero${i+1}><option value="">— choisir —</option>${heroes.map(h=>`<option value="${h.name}" ${h.name===defaults[i]?'selected':''}>${h.name}</option>`).join('')}</select><select id=teamLane${i+1}><option value=front ${lanes[i]==='front'?'selected':''}>Front</option><option value=mid ${lanes[i]==='mid'?'selected':''}>Mid</option><option value=back ${lanes[i]==='back'?'selected':''}>Back</option></select></div>`).join('');for(let i=1;i<=5;i++){document.getElementById('teamHero'+i).onchange=combat;document.getElementById('teamLane'+i).onchange=combat;}}
function dynamicTeamQ(){let q=[];for(let i=1;i<=5;i++){let h=document.getElementById('teamHero'+i),l=document.getElementById('teamLane'+i);if(h&&l){q.push('team'+i+'='+encodeURIComponent(h.value));q.push('lane'+i+'='+encodeURIComponent(l.value));}}q.push('aggro_front='+encodeURIComponent(aggroFront.value));q.push('aggro_mid='+encodeURIComponent(aggroMid.value));q.push('aggro_back='+encodeURIComponent(aggroBack.value));return q.join('&')}
async function loadCombatCustom(force=false){
 if(!force&&combatCustomLoadedHero===combatHero.value)return;
 let p=await getProfile(combatHero.value);
 setBuildValues('combatCustom',p.profile);
 for(let k of Object.keys(lk)){let e=document.getElementById('combatCustom_'+k);if(e)e.value=p.profile[k+'_level']||7}
 combatCustomLoadedHero=combatHero.value;
}
async function loadBossDynamic(){try{let d=await api('/api/boss-profile?name='+encodeURIComponent(simBoss.value));if(d.error){bossDynamicStatus.textContent=d.error;bossDynamicSummary.innerHTML='';bossDynamicActions.innerHTML='';return}bossDynamicStatus.textContent=(d.timings_resolved?'✓ Timings exacts résolus depuis '+(d.timing_source||'les données du jeu')+'.':'⚠ Profil mécanique chargé. Timings encore manquants : '+(d.missing_timings||[]).join(', '));bossDynamicSummary.innerHTML=cards({'Asset':d.asset_code||'—','Config ID':d.config_id||'—','Ouverture':(d.opening||[]).join(' → '),'Fin ouverture':F1(d.opening_end_s||0)+' s','Phases':(d.phases||[]).map(x=>x.trigger_hp_pct+'%').join(' / ')||'—'});let tl=d.opening_timeline||[];let acts=Object.values(d.actions||{});bossDynamicActions.innerHTML='<h4>Ouverture exacte</h4><table><tr><th>#</th><th>Action</th><th>Début</th><th>Boss disponible</th><th>Verrouillage</th><th>Animation</th></tr>'+tl.map(x=>`<tr><td>${x.n}</td><td><b>${x.label}</b></td><td>${F1(x.start_s)} s</td><td>${F1(x.available_s)} s</td><td>${F1(x.lock_s)} s</td><td>${F1(x.animation_s)} s</td></tr>`).join('')+'</table><h4>Compétences / mécaniques</h4><table><tr><th>Action</th><th>Cooldown</th><th>Cible</th><th>Effets</th><th>Timing</th></tr>'+acts.map(x=>{let t=x.timing||{};let ts=t.enchainement_s??t.lancement_s??'—';let an=t.animation_s??t.animation_totale_s??'—';return `<tr><td><b>${x.label||x.key}</b></td><td>${x.cooldown_s==null?'—':F1(x.cooldown_s)+' s'}</td><td>${x.target||'—'}</td><td>${(x.effects||[]).join(' · ')||'—'}</td><td>${ts==='—'?'—':F1(ts)+' s'} / anim ${an==='—'?'—':F1(an)+' s'}</td></tr>`}).join('')+'</table>'}catch(e){bossDynamicStatus.textContent='Erreur moteur boss : '+e.message}}
async function combat(){let b=bosses[simBoss.value];let elem=(simElement.value==='Auto'?b.element:simElement.value);let sups=[support1.value,support2.value,support3.value,support4.value];let preset=combatPreset.value;if(preset==='custom')await loadCombatCustom();let customQ=preset==='custom'?('&'+B('combatCustom')+'&'+LV('combatCustom')):'';let dtq=dynamicTeamQ();let d=await api(`/api/combat?name=${encodeURIComponent(combatHero.value)}&preset=${encodeURIComponent(preset)}&${dtq}&support1=${encodeURIComponent(sups[0])}&support2=${encodeURIComponent(sups[1])}&support3=${encodeURIComponent(sups[2])}&support4=${encodeURIComponent(sups[3])}&adds_mode=${encodeURIComponent(addsMode.value)}&boss_name=${encodeURIComponent(simBoss.value)}&duration=${simDur.value}&boss=${b.defense}&boss_res=${b.resistance}&boss_hp=${b.hp}&boss_atk=${b.attack}&element=${encodeURIComponent(elem)}${customQ}`);lastCombat=d;simHeroStats.innerHTML=statCards(d.preset_stats||d.final_stats||{});let lv=d.preset_levels||{};simHeroLevels.innerHTML=cards({'Auto':lv.auto==11?'♛':lv.auto,'S1':lv.s1==11?'♛':lv.s1,'S2':lv.s2==11?'♛':lv.s2,'S3':lv.s3==11?'♛':lv.s3,'Ult':lv.ult==11?'♛':lv.ult,'Preset':d.preset_label||preset});supportBuilds.innerHTML=(d.supports||[]).map(x=>`<div class=card><h3>${x.name}</h3><div class=muted>Élément ${x.element||'Neutre'} · ${x.element_matchup?.relation||'Neutre'}</div>${statCards(x.stats||{})}<div class=muted>Auto ${x.levels?.auto||'—'} · S1 ${x.levels?.s1||'—'} · S2 ${x.levels?.s2||'—'} · S3 ${x.levels?.s3||'—'} · Ult ${x.levels?.ult||'—'}<br>Débuffs: ${x.debuff_successes||0}/${x.debuff_attempts||0}</div></div>`).join('')||'<div class=note>Aucun buffer sélectionné.</div>';let sn=(d.supports||[]).map(x=>x.name).join(' + ')||'Aucun';combatSummary.innerHTML=cards({'Élément carry':d.hero_element||'Neutre','Matchup':d.element_matchup?.relation||'Neutre','Mod. dégâts':((d.element_matchup?.damage_mult||1)-1>=0?'+':'')+P((d.element_matchup?.damage_mult||1)-1),'Mod. Crit':(d.element_matchup?.crit_delta>=0?'+':'')+P(d.element_matchup?.crit_delta||0),'Mod. débuff':(d.element_matchup?.debuff_delta>=0?'+':'')+P(d.element_matchup?.debuff_delta||0),'DPS carry':F1(d.dps),'DPS sans buffers':F1(d.dps_without_supports||d.dps),'Gain buffers':(d.support_gain_dps>=0?'+':'')+F1(d.support_gain_dps||0),'Synergie / chevauchement':(d.support_synergy_dps>=0?'+':'')+F1(d.support_synergy_dps||0),'Dégâts':F(d.total_damage),'Actions carry':d.actions,'Buffers':sn});supportContribution.innerHTML='<table><tr><th>Buffer</th><th>DPS avec lui seul</th><th>Gain seul</th><th>Gain %</th><th>Apport marginal équipe</th><th>Buffs principaux</th></tr>'+((d.support_contributions||[]).map(x=>`<tr><td>${x.name}</td><td>${F1(x.dps_with_only)}</td><td>${x.gain_dps>=0?'+':''}${F1(x.gain_dps)}</td><td>${x.gain_pct>=0?'+':''}${P(x.gain_pct)}</td><td>${x.marginal_gain_dps>=0?'+':''}${F1(x.marginal_gain_dps)}</td><td>${(x.buffs||[]).map(b=>`${b.effect} (${P(b.uptime)})`).join(' · ')||'—'}</td></tr>`).join('')||'<tr><td colspan=6>Aucun buffer sélectionné.</td></tr>')+'</table>';teamBuffTable.innerHTML='<table><tr><th>Effet</th><th>Source active</th><th>Applications</th><th>Valeur max</th><th>Uptime</th></tr>'+((d.team_buffs||[]).map(x=>`<tr><td>${x.effect}</td><td>${x.sources||'—'}</td><td>${x.applications}</td><td>${P(x.max_value)}</td><td>${P(x.uptime)}</td></tr>`).join('')||'<tr><td colspan=5>Aucun buff équipe / débuff boss détecté.</td></tr>')+'</table>';combatNote.textContent=d.note;let sv=d.boss_survival||{};if(sv.enabled){teamSurvivalStatus.textContent=(sv.targeting_validated?'✓ Ciblage validé.':'⚠ '+(sv.targeting_note||'Ciblage provisoire.'))+(sv.missing_members?.length?' · Stats manquantes : '+sv.missing_members.join(', '):'');let dead=(sv.team||[]).filter(x=>!x.alive).length;teamSurvivalCards.innerHTML=cards({'Membres simulés':(sv.team||[]).length,'Morts ouverture':dead,'ATQ boss':F(sv.boss_atk||0),'Portée':sv.scope||'—'});teamSurvivalTable.innerHTML='<table><tr><th>Héros</th><th>Ligne</th><th>Classe</th><th>PV max</th><th>DEF</th><th>PV fin ouverture</th><th>PV %</th><th>Dégâts reçus</th><th>Corruption</th><th>État</th><th>Source stats</th></tr>'+((sv.team||[]).map(x=>`<tr><td><b>${x.name}</b></td><td>${x.lane||'—'}</td><td>${x.class||'—'}</td><td>${F(x.max_hp)}</td><td>${F(x.defense)}</td><td>${F(x.hp)}</td><td>${P(x.hp_pct)}</td><td>${F(x.damage_taken)}</td><td>${x.corruption||0}</td><td>${x.alive?'Vivant':'Mort à '+F1(x.death_s)+' s'}</td><td>${x.source||'—'}</td></tr>`).join(''))+'</table>';teamSurvivalEvents.innerHTML='<h4>Dégâts entrants</h4><table><tr><th>Temps</th><th>Action</th><th>% ATQ brut</th><th>Ciblage</th><th>Cibles / dégâts</th></tr>'+((sv.events||[]).map(e=>`<tr><td>${F1(e.time)} s</td><td><b>${e.label}</b></td><td>${e.raw_pct_atk==null?'—':F1(e.raw_pct_atk)+'%'}</td><td>${e.targeting||'—'}${e.target_proxy?' (proxy)':''}</td><td>${(e.targets||[]).map(t=>t.name+' -'+F(t.damage)+' PV').join(' · ')||e.note||'—'}</td></tr>`).join(''))+'</table>';}else{teamSurvivalStatus.textContent=sv.reason||'Survie boss non disponible.';teamSurvivalCards.innerHTML='';teamSurvivalTable.innerHTML='';teamSurvivalEvents.innerHTML='';}let globalRows=[];(d.log||[]).forEach(x=>globalRows.push({t:+x.time||0,actor:d.hero||combatHero.value,action:x.action,duration:+x.duration||0,kind:'Héros'}));(((d.boss_dynamic||{}).opening_timeline)||[]).forEach(x=>globalRows.push({t:+x.start_s||0,actor:(d.boss_dynamic||{}).display_name||simBoss.value,action:x.label,duration:+x.lock_s||0,kind:'Boss'}));globalRows.sort((a,b)=>a.t-b.t||((a.kind==='Boss')?-1:1));globalCombatTable.innerHTML='<table><tr><th>Temps</th><th>Acteur</th><th>Type</th><th>Action</th><th>Durée / verrouillage</th></tr>'+globalRows.map(x=>`<tr><td>${F1(x.t)} s</td><td><b>${x.actor}</b></td><td>${x.kind}</td><td>${x.action}</td><td>${F1(x.duration)} s</td></tr>`).join('')+'</table>';combatTable.innerHTML='<table><tr><th>#</th><th>Temps</th><th>Action</th><th>Durée</th><th>Hits</th><th>Crits</th><th>Dégâts</th><th>Mana</th><th>Effets</th></tr>'+d.log.map(x=>`<tr><td>${x.n}</td><td>${F1(x.time)}</td><td>${x.action}</td><td>${F1(x.duration)}</td><td>${x.hits}</td><td>${x.crits}</td><td>${F(x.damage)}</td><td>${F1(x.mana_after)}</td><td>${x.effects}</td></tr>`).join('')+'</table>';renderEffects(d)}
...
async function compare(){let b=bosses[simBoss.value],preset=combatPreset.value,elem=(simElement.value==='Auto'?b.element:simElement.value);let comparePreset=preset==='custom'?'box':preset;let d=await api(`/api/compare?a=${encodeURIComponent(aSel.value)}&b=${encodeURIComponent(bSel.value)}&preset=${encodeURIComponent(comparePreset)}&duration=${simDur.value}&boss=${b.defense}&boss_res=${b.resistance}&boss_hp=${b.hp}&boss_atk=${b.attack}&element=${encodeURIComponent(elem)}`);let labels={box:'Ma box',early:'Early game',mid:'Mid game',late:'Late game'};comparePresetNote.textContent=preset==='custom'?'Le mode Personnalisé concerne la Simulation de combat. Le Comparateur reste sur Ma box.':`Preset : ${labels[preset]||preset} — même boss et même durée que la Simulation de combat.`;cmpWinner.textContent=d.winner?`${d.winner} devant de ${F1(d.diff_dps)} DPS (${P(d.pct)})`:'Égalité';cmpGrid.innerHTML=[d.a,d.b].map(x=>`<div class=card><h3>${x.hero}</h3><div class=big>${F1(x.dps)} DPS</div>${statCards(x.final_stats)}<div class=muted>Auto/S1/S2/S3/Ult : ${['auto','s1','s2','s3','ult'].map(k=>x.preset_levels?.[k]===11?'♛':(x.preset_levels?.[k]??'—')).join(' / ')}</div><hr>Autos ${F(x.damage_by.Auto)} · S1 ${F(x.damage_by['Skill 1'])} · S2 ${F(x.damage_by['Skill 2'])} · S3 ${F(x.damage_by['Skill 3'])} · Ult ${F(x.damage_by.Ultimate)}</div>`).join('')}
...
async function bestSupports(){
 ...
 try{
  ...
  window.lastBestSupportData=d;
  renderBestSupportList();
  let t=d.best_trio;
  if(t){
...
heroSel.onchange=hero;combatHero.onchange=async()=>{combatCustomLoadedHero='';renderDynamicTeam();if(combatPreset.value==='custom')await loadCombatCustom(true);await combat()};combatPreset.onchange=async()=>{let notes={box:'Ma box : stats et niveaux réellement importés.',custom:'Personnalisé : les valeurs saisies ci-dessous sont utilisées directement pour ce combat.',early:'Early : skills 1 · ATQ +30% · Crit 20% · Dég crit 50% · PRE +80 · Combo/Skill/Recovery/Mana +5%.',mid:'Mid : skills 5 · ATQ +80% · Crit 50% · Dég crit 75% · PRE +220 · Combo/Skill/Recovery/Mana +15%.',late:'Late : skills max · ATQ +150% · Crit 100% · Dég crit 120% · PRE +400 · Combo/Skill/Recovery/Mana +30%.'};combatPresetNote.textContent=notes[combatPreset.value]||'';combatCustom.classList.toggle('hidden',combatPreset.value!=='custom');if(combatPreset.value==='custom')await loadCombatCustom();await combat();compare()};aSel.onchange=compare;bSel.onchange=compare;build(combatCustomStats,'combatCustom');levels(combatCustomLevels,'combatCustom');recFields(recCurrentStats,'recCur');recTestFields(recTestStats,'recTest');optHero.onchange=async()=>{await loadRecStats();await critAnalysis();await recAnalysis();await relicPotential()};[...new Set(heroes.map(x=>x.rarity).filter(Boolean))].sort().forEach(x=>rarity.add(new Option(x,x)));[...new Set(heroes.map(x=>x.role).filter(Boolean))].sort().forEach(x=>role.add(new Option(x,x)));rankMode.onchange=()=>{let notes={box:'Ma box : classement avec les builds réellement importés.',early:'Early : skills 1 · ATQ +30% · Crit 20% · Dég crit 50% · PRE +80 · Combo/Skill/Recovery/Mana +5%.',mid:'Mid : skills 5 · ATQ +80% · Crit 50% · Dég crit 75% · PRE +220 · Combo/Skill/Recovery/Mana +15%.',late:'Late : skills max · ATQ +150% · Crit 100% · Dég crit 120% · PRE +400 · Combo/Skill/Recovery/Mana +30%.'};rankModeNote.textContent=notes[rankMode.value]||'';rank()};combatBtn.onclick=combat;combatCustomReload.onclick=async()=>{await loadCombatCustom(true);await combat()};bestSupportMode.onchange=()=>{let notes={real:'Ma box : uniquement les héros que tu possèdes, avec leur vraie fiche importée.',base:'Stats de base : stats natives du héros et tous les skills niveau 1.',max:'Max support : stats natives, skills max, PRE 1000, Combo/Skill Speed/Recovery/Mana +30%.'};bestSupportModeNote.textContent=notes[bestSupportMode.value]||''};bestSupportLimit.onchange=renderBestSupportList;bestSupportBtn.onclick=bestSupports;cmpBtn.onclick=compare;critBtn.onclick=critAnalysis;recBtn.onclick=recAnalysis;recBalanceBtn.onclick=recBalance;relicOptBtn.onclick=relicOptimize;setOptBtn.onclick=setOptimize;potentialBtn.onclick=relicPotential;rankBtn.onclick=rank;aoeHero.onchange=async()=>{await loadAoeDefaults();await aoeCombatRun()};aoePreset.onchange=()=>{aoeCombatRun();aoeRankRun()};aoeEnemies.onchange=async()=>{await loadAoeDefaults();await aoeCombatRun();await aoeRankRun()};aoeBoss.onchange=()=>{aoeCombatRun();aoeRankRun()};aoeBtn.onclick=aoeCombatRun;aoeRankBtn.onclick=aoeRankRun;aoeSaveTargets.onclick=saveAoeTargets;aoeProbeStatic.onclick=probeAoeStatic;aoeImportAll.onclick=importAllAoe;importBoxBtn.onclick=importBoxOneClick;relicRefresh.onclick=loadRelics;[relicSlot,relicSet,relicEquipped,relicStat].forEach(e=>e.onchange=loadRelics);await loadRelics();await hero();await loadRecStats();await loadTrophyHeroes();await combat();await compare();await critAnalysis();await recAnalysis();await relicPotential();await loadAoeDefaults();await aoeCombatRun();await aoeRankRun();await rank()})().catch(e=>document.body.insertAdjacentHTML('beforeend',`<pre>${e.stack}</pre>`));
</script></body></html>'''
...
class H(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        try:
            ...
            if p.path=='/api/boss-profile':
                if not _get_boss_profile or not _boss_profile_as_dict:
                    self.sendj({'error':'Moteur boss dynamique indisponible'},500); return
                bp=_get_boss_profile(qs.get('name',[''])[0])
                if not bp:
                    self.sendj({'error':'Aucun profil dynamique enregistré pour ce boss'},404); return
                self.sendj(_boss_profile_as_dict(bp,{})); return
            if p.path=='/api/combat':
                name=qs.get('name',[''])[0]; preset=qs.get('preset',['box'])[0]
                if str(preset).strip().lower()=='custom':
                    b,st=manual_build_for(name)
                    combat_lv={k:max(1,min(11,int(float(qs.get(k,['7'])[0])))) for k in ('auto','s1','s2','s3','ult')}
                    preset_label='Personnalisé'; preset_key='custom'
                else:
                    b,st,combat_lv,preset_label,preset_key=combat_preset_for(name,preset)
                supports=[qs.get(f'support{i}',['Aucun'])[0] for i in range(1,5)]; dur=f('duration',120); r=simulate_combat(name,combat_lv,dur,f('boss',1320),f('boss_res',0),f('boss_hp',0),f('boss_atk',0),qs.get('element',['Neutre'])[0],**b,analyze_effect_gains=True,team_supports=supports,adds_mode=qs.get('adds_mode',['none'])[0]);
                if r:
                    _bn=qs.get('boss_name',[''])[0]
                    if _get_boss_profile and _boss_profile_as_dict:
                        _bp=_get_boss_profile(_bn)
                        if _bp:
                            r['boss_dynamic']=_boss_profile_as_dict(_bp,{})
                            if _simulate_opening_survival:
                                _team=[]
                                for _i in range(1,6):
                                    _hn=qs.get(f'team{_i}',[name if _i==1 else ''])[0]
                                    if not _hn: continue
                                    _lane=qs.get(f'lane{_i}',['mid'])[0]
            ...
            if p.path=='/api/best-supports':
                ...
                for c in candidates:
                    ...
                    eval_profile=eval_profiles.get(sn)
                    has_advanced=bool(eval_profile and getattr(eval_profile,'advanced_effects',None))
                    if not events and not has_advanced: continue
                    ...
                    gain_pct=(gain/base_dps if base_dps else 0)
                    eval_raw=None
                    if eval_profile is not None and _evaluate_support_impacts is not None:
                        try:
                            eval_raw=_evaluate_support_impacts(
                                eval_profile,
                                gain_pct*100.0,
                                dur,
                                _carry_hp,
                ...
                # - boss : offense + survie calibrée sur le boss si un TTD de référence existe.
                if results and _support_percentile_scores:
                    _off_scores=_support_percentile_scores({x['name']:num(x.get('gain_pct')) for x in results})
                    _general_scores=_support_percentile_scores({x['name']:num((x.get('support_eval_raw') or {}).get('general_impact_pct')) for x in results})
                    _boss_scores=_support_percentile_scores({x['name']:num((x.get('support_eval_raw') or {}).get('boss_impact_pct')) for x in results})
                else:
                    _off_scores={}; _general_scores={}; _boss_scores={}
                for x in results:
                    x['score_offensif_100']=_off_scores.get(x['name'])
                    x['score_general_100']=_general_scores.get(x['name'])
                    x['score_boss_100']=_boss_scores.get(x['name'])
                    x['impact_general_pct']=num((x.get('support_eval_raw') or {}).get('general_impact_pct'))
                    x['impact_boss_pct']=num((x.get('support_eval_raw') or {}).get('boss_impact_pct'))
                    x['support_eval']=support_eval_payload(eval_profiles.get(x['name']),boss_name)
                    if x['support_eval'] is not None:
                        x['support_eval']['score_general_100']=x['score_general_100']
                        x['support_eval']['score_boss_100']=x['score_boss_100']
                        x['support_eval']['impact_general_pct']=x['impact_general_pct']
                ...
                self.sendj({'hero':name,'base_dps':base_dps,'tested':len(results),'support_mode':support_mode,'boss_name':boss_name,'top3':results[:3],'top20':results[:20],'best_trio':best_trio,'second_trio':second_trio,'trio_pool':len(trio_candidates),'score_status':'offense_plus_reference_survival'}); return
