...
let tacticNames={11:'Standard',12:'Mur de Pierre',13:'Tour',14:'Avant-garde',15:'Phalange',16:'Fer de lance',17:'Siège',18:'Artillerie',19:'Tireur d’élite'};
function posLane(p){p=+p;return p<20?'front':p<30?'mid':'back'}
function tacticBonusLabel(b){let names={def_pct:'DEF',hp_pct:'PV',atk_pct:'ATQ',crit_rate:'Taux crit.',accuracy:'PRE',resistance:'RÉS',combo_speed_points:'VIT combo',skill_speed_points:'VIT compétence',skill_recovery_points:'RÉCUP comp.',mana_gen_points:'GÉN mana',damage_taken_pct:'Dégâts subis',heal_amount_pct:'Soins',shield_value_pct:'Bouclier',def_ignore_pct:'DEF ignorée'};let pct=['def_pct','hp_pct','atk_pct','crit_rate','damage_taken_pct','heal_amount_pct','shield_value_pct','def_ignore_pct'].includes(b.stat);let v=pct?((b.value>=0?'+':'')+F1(b.value*100)+'%'):((b.value>=0?'+':'')+F1(b.value));return (names[b.stat]||b.stat)+' '+v+(b.conditional_class?' ['+b.conditional_class+']':'')}
