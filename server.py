...
def apply_box_profile(name):
    """Replace one saved profile with the exact imported-box build, preserving skill/titan choices when possible."""
    bx=box_hero_for_name(name)
    bb=(bx or {}).get('box_build') or {}
    fs=bb.get('final_stats')
    ...
    rec={
        ...
        'auto_level':int(num(old.get('auto_level'),7)),'s1_level':int(num(old.get('s1_level'),7)),
        's2_level':int(num(old.get('s2_level'),7)),'s3_level':int(num(old.get('s3_level'),7)),
        'ult_level':int(num(old.get('ult_level'),7)),
...
</style></head><body><header><h1>🧪 Smishie's Lab</h1><div class=muted>V10.73 — Correction profil box</div></header>
...
if __name__=='__main__':
    print("Smishie's Lab V10.73 — Correction profil box — http://127.0.0.1:8501")
