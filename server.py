...
# Courbes exactes extraites de StaticCharacterData.AscensionMultipliersByRank.
ASCENSION_MULTIPLIERS = {
 1:[1.00,1.05,1.10,1.16,1.21,1.27,1.33,1.40,1.46,1.53],
 2:[1.05,1.10,1.16,1.21,1.27,1.33,1.40,1.46,1.53,1.61,1.68,1.76,1.84,1.92,2.00,2.09,2.18,2.27,2.37,2.46],
 3:[1.10,1.15,1.21,1.27,1.33,1.39,1.46,1.53,1.61,1.68,1.76,1.84,1.93,2.01,2.10,2.20,2.29,2.39,2.49,2.60,2.71,2.82,2.93,3.05,3.16,3.29,3.41,3.54,3.67,3.80],
 4:[1.15,1.20,1.26,1.32,1.39,1.45,1.53,1.60,1.68,1.76,1.84,1.93,2.02,2.11,2.21,2.31,2.41,2.51,2.62,2.74,2.85,2.97,3.09,3.22,3.35,3.48,3.61,3.75,3.89,4.04,4.19,4.34,4.49,4.65,4.81,4.97,5.14,5.31,5.49,5.66],
 5:[1.20,1.26,1.32,1.38,1.44,1.52,1.59,1.67,1.75,1.83,1.92,2.01,2.11,2.21,2.31,2.42,2.52,2.64,2.76,2.88,3.00,3.13,3.26,3.39,3.53,3.68,3.82,3.97,4.12,4.28,4.44,4.60,4.77,4.94,5.12,5.30,5.48,5.66,5.85,6.04,6.24,6.44,6.64,6.85,7.06,7.28,7.49,7.71,7.94,8.17],
...
def _load_extracted_progression():
    ...
    node_key={'attack_pct':'atk_pct','defense_pct':'def_pct','health_pct':'health_pct','critical_damage':'crit_dmg',
              'critical_rate':'crit_rate','accuracy':'accuracy','resistance':'resistance','run_speed':'run_speed',
    ...
    for config_id,row in chars.items():
        ...
        HERO_PROGRESSION[str(config_id)]={'name':row.get('name'),'soul_sign':int(sign),'nodes':nodes,
          ...
          'awake_base':{'health':ab.get('health'),'atk':ab.get('attack'),'defense':ab.get('defense'),'run_speed':ab.get('run_speed'),
                        'crit_rate':ab.get('critical_rate'),'crit_dmg':ab.get('critical_damage'),'accuracy':ab.get('accuracy'),
                        'resistance':ab.get('resistance'),'instinct':ab.get('instinct'),'combo_points':ab.get('combo_speed'),
...
def box_build_for_instance(name,hero_instance):
    ...
    if cfg:
        ...
        if 1 <= level <= len(curve):
            ...
            naked={'health':round((num(base.get('health'))+num(awb.get('health')))*10*mult),
                   ...
                   'skill_recovery':skill_points_to_pct(num(base.get('skill_recovery_points'))+num(awb.get('skill_recovery_points'))),
                   'mana_gen':(num(base.get('mana_points'))+num(awb.get('mana_points')))/100.0}
            ...
            for node_id in node_ids:
                ...
                naked['mana_gen']+=num(n.get('mana_points'))/100.0
    ...
    final={
        ...
        'mana_gen':num(h.get('mana_gen'))+b['mana_points']/100.0,
    }
...
def _stats_without_relics(name, raw_profile, current_relics, profile_is_saved):
    ...
    return {
        ...
        'mana_gen':max(0.0,num(raw_profile.get('mana_gen'))-b['mana_points']/100.0),
    }
...
def _bonus_add(a,b):
    out={}
    for k in ('atk_flat','atk_pct','crit_rate','crit_dmg','accuracy','resistance','combo_points','skill_speed_points','skill_recovery_points','mana_points','hp_pct','def_pct','instinct'):
        out[k]=num(a.get(k))+num(b.get(k))
    out['unapplied']={}
    return out

def optimize_relics_for_dps(name,duration=120,boss_def=1320,boss_res=0,boss_hp=0,boss_atk=0,boss_element='Neutre',mode='all',top_per_slot=28,beam_width=350,sim_candidates=60):
    hero=box_hero_for_name(name)
    if not hero:
    ...
    return {
        ...
        'collection_relics':len(all_relics),'candidates_per_slot':{str(k):len(v) for k,v in byslot.items()},'simulated_combinations':len(tested),
...
def profile_for(name):
    ...
    for k,v in store.items():
        if k.lower()==key and isinstance(v,dict):
            out=dict(v); out['element']=normalize_element(out.get('element') or h.get('element'))
            return out
    ...
    if r:
        ...
        store[str(name)]=r; _write_profile_store(store)
        return r
    return {
        ...
        'element':normalize_element(h.get('element'))
    }
...
def simulate_combat(name, levels=None, duration=120, boss_def=1320, boss_res=0, boss_hp=0, boss_atk=0, boss_element='Neutre', atk_pct=1.5, cr_add=.7, cd_add=1.4, combo_add=.25, skill_speed_add=.2, recovery_add=.2, mana_add=.2, acc_add=340, resistance_final=None, comparison_mode=False, disabled_effects=None, analyze_effect_gains=False, team_supports=None, disabled_team_buffs=None, adds_mode='none', prepared_team=None):
    ...
    combo_base=auto_chain_cycle_base(name,mr)
    base_mana_gen=1+num(h.get('mana_gen'))+mana_add; acc=num(h.get('accuracy'))+acc_add
    ...
    if analyze_effect_gains and eff:
        ...
        for e in eff:
            nm=e['effect']
            cf=simulate_combat(name,lv,duration,boss_def,boss_res,boss_hp,boss_atk,boss_element,atk_pct,cr_add,cd_add,combo_add,skill_speed_add,recovery_add,mana_add,acc_add,resistance_final,comparison_mode,disabled_effects|{nm},False,team_supports,disabled_team_buffs,adds_mode)
...
</style></head><body><header><h1>🧪 Smishie's Lab</h1><div class=muted>V10.65 — Progression exacte généralisée</div></header>
...
<section id=sim class=hidden><div class=subnav><button class="subtab active" data-sub="combat">Combat</button><button class=subtab data-sub="effects">Analyse effets</button><button class=subtab data-sub="compare">Comparateur</button><button class=subtab data-sub="bestSupport">Meilleur support</button></div>
<div id=combat>
<h3>Stats du boss</h3><div class=controls><div class=control><label>Boss</label><select id=simBoss></select></div><div class=control><label>Durée</label><input id=simDur type=number value=120></div></div><div id=simBossCards class=grid></div>
<h3>Build de la fiche héros</h3><div class=controls><div class=control><label>Héros principal</label><select id=combatHero></select></div></div><h4>Stats du héros principal</h4><div id=simHeroStats class=grid></div><h4>Niveaux des skills</h4><div id=simHeroLevels class=grid></div>
...
<div class=controls><div class=control><label>Profil supports</label><select id=bestSupportMode><option value=real>Mode réel — fiches sauvegardées</option><option value=best>Best Support — PRE 1000 / Recovery 30% / Skill Speed 30% / skills max</option></select></div></div>
...
<div id=compare class=hidden><div class=controls><div class=control><label>Héros 1</label><select id=aSel></select></div><div class=control><label>Héros 2</label><select id=bSel></select></div><button id=cmpBtn>Comparer</button></div><div class=compare-edit><div class=card><h3>Fiche héros 1</h3><div id=aBuild class=levels></div><div id=aLevels class=levels></div><button id=saveA>Enregistrer fiche 1</button><div id=aSaveStatus class=good></div></div><div class=card><h3>Fiche héros 2</h3><div id=bBuild class=levels></div><div id=bLevels class=levels></div><button id=saveB>Enregistrer fiche 2</button><div id=bSaveStatus class=good></div></div></div><div id=cmpWinner class=good></div><div id=cmpGrid class=grid></div></div></section>
...
async function bestSupports(){
 let b=bosses[simBoss.value]; bestSupportBtn.disabled=true; bestSupportBtn.textContent='Analyse en cours…'; bestSupportStatus.textContent='Recherche des buffers/debuffers utiles dans tous les rôles…'; bestSupportTop.innerHTML=''; bestSupportTrio.innerHTML='';
 try{
  let d=await api(`/api/best-supports?name=${encodeURIComponent(combatHero.value)}&duration=${simDur.value}&boss=${b.defense}&boss_res=${b.resistance}&boss_hp=${b.hp}&boss_atk=${b.attack}&element=${encodeURIComponent(b.element)}&support_mode=${encodeURIComponent(bestSupportMode.value)}`);
  bestSupportStatus.textContent=`${d.tested||0} buffers/debuffers utiles testés pour ${d.hero} contre ${simBoss.value}. Profil : ${d.support_mode==='best'?'Best Support normalisé':'Réel / fiches sauvegardées'}. DPS seul : ${F1(d.base_dps)}.`;
...
async function relicOptimize(){
 ...
 try{
  let d=await api(`/api/relic-optimize?name=${encodeURIComponent(optHero.value)}&mode=${encodeURIComponent(relicOptMode.value)}&duration=${optDur.value}&boss=${b.defense}&boss_res=${b.resistance}&boss_hp=${b.hp}&boss_atk=${b.attack}&element=${encodeURIComponent(b.element)}`);
  ...
  relicOptStats.innerHTML=statCards(d.best?.stats||{})+cards({'Sets actuel':(d.current?.sets||[]).map(x=>x.name+' '+x.tiers.join('+')+(x.total?' ('+x.total+')':'')).join(' · ')||'Aucun','Sets optimisés':(d.best?.sets||[]).map(x=>x.name+' '+x.tiers.join('+')+(x.total?' ('+x.total+')':'')).join(' · ')||'Aucun'});
    ...
    def do_GET(self):
        ...
        try:
            ...
            if p.path=='/api/auto-timings':
                ...
                if row:self.js({'hero':name,'source':'extracted','timings':row})
                else:self.js({'hero':name,'source':'legacy_fallback','timings':None,'cycle_base_s':auto_chain_cycle_base(name)})
                return
            if p.path=='/api/combat':
                name=qs.get('name',[''])[0]; b,st,pr=saved_build_for(name); supports=[qs.get(f'support{i}',['Aucun'])[0] for i in range(1,5)]; dur=f('duration',120); r=simulate_combat(name,profile_levels(pr),dur,f('boss',1320),f('boss_res',0),f('boss_hp',0),f('boss_atk',0),qs.get('element',['Neutre'])[0],**b,analyze_effect_gains=True,team_supports=supports,adds_mode=qs.get('adds_mode',['none'])[0]);
                if r:
                    base=simulate_combat(name,profile_levels(pr),dur,f('boss',1320),f('boss_res',0),f('boss_hp',0),f('boss_atk',0),qs.get('element',['Neutre'])[0],**b)
                    ...
                    for sn in active_supports:
                        one=simulate_combat(name,profile_levels(pr),dur,f('boss',1320),f('boss_res',0),f('boss_hp',0),f('boss_atk',0),qs.get('element',['Neutre'])[0],**b,team_supports=[sn],adds_mode=qs.get('adds_mode',['none'])[0])
                        if one: single_sims[sn]=one
                    for sn in active_supports:
                        one=single_sims.get(sn)
                        other_names=[x for x in active_supports if x!=sn]
                        if other_names:
                            other=single_sims.get(other_names[0]) if len(other_names)==1 else simulate_combat(name,profile_levels(pr),dur,f('boss',1320),f('boss_res',0),f('boss_hp',0),f('boss_atk',0),qs.get('element',['Neutre'])[0],**b,team_supports=other_names,adds_mode=qs.get('adds_mode',['none'])[0])
            ...
            if p.path=='/api/compare':
                common=(f('duration',120),f('boss',1320),f('boss_res',0),f('boss_hp',0),f('boss_atk',0),qs.get('element',['Neutre'])[0]); an=qs.get('a',[''])[0]; bn=qs.get('b',[''])[0]; ab,_=manual_build_for(an,'a_'); bb,_=manual_build_for(bn,'b_'); a=simulate_combat(an,levels('a_'),*common,**ab); b=simulate_combat(bn,levels('b_'),*common,**bb)
                if not a or not b:self.sendj({'error':'données manquantes'},400);return
                diff=a['dps']-b['dps']; self.sendj({'a':a,'b':b,'winner':a['hero'] if diff>0 else b['hero'] if diff<0 else None,'diff_dps':abs(diff),'pct':abs(diff)/max(1,min(a['dps'],b['dps']))}); return
            if p.path=='/api/relic-optimize':
                name=qs.get('name',[''])[0]; mode=qs.get('mode',['all'])[0]
                r=optimize_relics_for_dps(name,f('duration',120),f('boss',1320),f('boss_res',0),f('boss_hp',0),f('boss_atk',0),qs.get('element',['Neutre'])[0],mode)
            ...
            if p.path=='/api/rank':
                rr=qs.get('rarity',['Toutes'])[0]; role=qs.get('role',['Tous'])[0]; mode=qs.get('mode',['box'])[0]; out=[]
                ...
                prepared_team=None
                if supports:
                    pe,pi=prepare_team_buffs(supports,duration,'none',br,elem)
                ...
                for h in q('SELECT name,faction,rarity,role,element FROM heroes WHERE name IS NOT NULL'):
                    ...
                    if mode=='box':
                        if not has_saved_profile(h['name']): continue
                        bld,_,pr=saved_build_for(h['name']); lv=profile_levels(pr)
                    elif mode=='base':
                        hr=hero_row(h['name']) or {}; st={'atk':num(hr.get('atk')),'crit_rate':num(hr.get('crit_rate')),'crit_dmg':num(hr.get('crit_dmg')),'accuracy':num(hr.get('accuracy')),'resistance':num(hr.get('resistance')),'combo_speed':num(hr.get('combo_speed')),'skill_speed':num(hr.get('skill_speed')),'skill_recovery':num(hr.get('skill_recovery')),'mana_gen':num(hr.get('mana_gen'))}; bld=final_to_build(h['name'],st); lv={'auto':7,'s1':7,'s2':7,'s3':7,'ult':7}
                    else:
                        hr=hero_row(h['name']) or {}; st={'atk':num(hr.get('atk'))*(1+f('end_atk',150)/100),'crit_rate':f('end_cr',1),'crit_dmg':f('end_cd',2),'accuracy':f('end_pre',475),'resistance':f('end_res',100),'combo_speed':f('end_combo',.20),'skill_speed':f('end_speed',.20),'skill_recovery':f('end_rec',.20),'mana_gen':f('end_mana',.20)}; bld=final_to_build(h['name'],st); el=int(f('end_lvl',10)); lv={'auto':el,'s1':el,'s2':el,'s3':el,'ult':el}
...
if __name__=='__main__':
    print("Smishie's Lab V10.65 — Progression exacte généralisée — http://127.0.0.1:8501")
